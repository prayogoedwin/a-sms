<x-layouts.app>
    <div class="mb-6"><h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Edit Jadwal</h1></div>
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 max-w-2xl">
        <form method="POST" action="{{ route('master-data.penjadwalan.update', $jadwal) }}" class="space-y-3">
            @csrf
            @method('PUT')
            <div>
                ...
                <select name="kelas_id" class="w-full px-4 py-1.5 rounded-lg text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600" required>
                    ...
                    @foreach($kelas as $item)
                        <option value="{{ $item->id }}" @selected(old('kelas_id', $jadwal->kelas_id) == $item->id)>{{ $item->tingkat->nama }} {{ $item->nama_kelas }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <div>
                    ...
                    <select name="semester" class="w-full px-4 py-1.5 rounded-lg text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600">
                        <option value="ganjil" @selected(old('semester', $jadwal->semester) === 'ganjil')>Ganjil</option>
                        <option value="genap" @selected(old('semester', $jadwal->semester) === 'genap')>Genap</option>
                    </select>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
                <x-forms.input label="Jam Mulai" name="jam_mulai" type="time" value="{{ old('jam_mulai', $jadwal->jam_mulai) }}" required />
                <x-forms.input label="Jam Selesai" name="jam_selesai" type="time" value="{{ old('jam_selesai', $jadwal->jam_selesai) }}" required />
            </div>
            <div>
                ...
                <select name="tahun_ajaran_id" class="w-full px-4 py-1.5 rounded-lg text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600" required>
                    ...
                    @foreach($tahunAjarans as $ta)
                        <option value="{{ $ta->id }}" @selected(old('tahun_ajaran_id', $jadwal->tahun_ajaran_id) == $ta->id)>{{ $ta->nama }}</option>
                    @endforeach
                </select>
            </div>
